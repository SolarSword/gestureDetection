...
path = 'data/applause_twice/applause_twice_2_.csv'
...
for i in range(0, len(segments)):
    ...
    accel_x = list(map(lambda a : a.value[0], segment))
    accel_y = list(map(lambda a : a.value[1], segment))
    accel_z = list(map(lambda a : a.value[2], segment))

    plt.plot(x_axis, accel, label = 'magnitude')
    plt.plot(x_axis, accel_x, '--', label = 'accel_x')
    plt.plot(x_axis, accel_y, '-.', label = 'accel_y')
    plt.plot(x_axis, accel_z, ':', label = 'accel_z')
